...
func (p *Postgres) TablesToStruct(tablenames []string) (map[string]*TemplateParams, error) {
	tables, err := p.RetrieveTables(tablenames)
	if err != nil {
		return nil, err
	}
	...
	for _, table := range tables {
		pkeys, err := p.RetrievePrimaryKeys(table)
		if err != nil {
			return nil, err
		}

		fields, err := p.RetrieveFields(table)
		if err != nil {
			return nil, err
		}
